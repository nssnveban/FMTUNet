...
FOLDER = "/root/autodl-tmp/SSRS/ISPRS_dataset/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
...
def metrics(predictions, gts, label_values=LABELS):
    ...
    for l_id, score in enumerate(F1Score):
        print("%s: %.4f" % (label_values[l_id], score))
    mean_F1Score = np.nanmean(F1Score[:5])
    print('mean F1Score: %.4f' % (mean_F1Score))
    print("---")
    ...
    MIoU_per_class = np.diag(cm) / (np.sum(cm, axis=1) + np.sum(cm, axis=0) - np.diag(cm))
    print(MIoU_per_class)
    mean_MIoU = np.nanmean(MIoU_per_class[:5])
    print('mean MIoU: %.4f' % (mean_MIoU))
    print("---")

    return {'accuracy': accuracy, 'mean_F1Score': mean_F1Score, 'mean_IoU': mean_MIoU}
